<#
PowerShell helper to start/stop/check a local server on port 3000.
Place this file in your project root (same folder as `server.js` and `package.json`).
Usage examples:
  .\start-localhost3000.ps1 -Action start
  .\start-localhost3000.ps1 -Action stop
  .\start-localhost3000.ps1 -Action status
  .\start-localhost3000.ps1 -Action restart
  .\start-localhost3000.ps1 -Action open
#>
...
function Open-Browser {
    # Open HTTPS main port (self-signed) and HTTP fallback port for convenience
    try { Start-Process "https://localhost:3000" } catch { }
    try { Start-Process "http://localhost:3002" } catch { }
}
